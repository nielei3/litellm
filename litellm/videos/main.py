...
async def avideo_extension(
    ...
    **kwargs,
    ...
    """
    Asynchronously calls the `video_extension` function with the given arguments and keyword arguments.

    Parameters:
    - `video_id` (str): The identifier of the completed video to extend
    - `prompt` (str): Text prompt that directs the extension generation
    - `seconds` (Optional[str]): Duration of the extension in seconds
    - `timeout` (int): Request timeout in seconds
    - `custom_llm_provider` (Optional[str]): The LLM provider to use
    - `extra_headers` (Optional[Dict[str, Any]]): Additional headers
    - `extra_query` (Optional[Dict[str, Any]]): Additional query parameters
    ...
    Returns:
    - `response` (VideoObject): The response returned by the `video_extension` function.
    """
    local_vars = locals()
    try:
        loop = asyncio.get_event_loop()
...
# Overload for when avideo_extension=True (returns Coroutine)
@overload
def video_extension(
    video_id: str,
    prompt: str,
...
def video_extension(
    ...
    custom_llm_provider=None,
    *,
    avideo_extension: Literal[False] = False,
    **kwargs,
) -> VideoObject:
    ...
...
@client
def video_extension(  # noqa: PLR0915
    video_id: str,
    prompt: str,
    seconds: Optional[str] = None,
    timeout=600,  # default to 10 minutes
    ...
    **kwargs,
    ...
    """
    Maps the https://api.openai.com/v1/videos/extensions endpoint.
    ...
    try:
        ...
        # get provider config
        video_extension_provider_config: Optional[
            BaseVideoConfig
        ] = ProviderConfigManager.get_provider_video_config(
            model=None,
            provider=litellm.LlmProviders(custom_llm_provider),
        ...
        if video_extension_provider_config is None:
            raise ValueError(f"video extension is not supported for {custom_llm_provider}")

        local_vars.update(kwargs)
        # For video extension, we need the video_id, prompt, and seconds
        video_extension_request_params: Dict = {
            "video_id": video_id,
            "prompt": prompt,
            "seconds": seconds,
        }
        ...
        litellm_logging_obj.update_environment_variables(
            model="",
            user=kwargs.get("user"),
            optional_params=dict(video_extension_request_params),
            litellm_params={
                "litellm_call_id": litellm_call_id,
                **video_extension_request_params,
            },
            custom_llm_provider=custom_llm_provider,
        )

        # Set the correct call type for video extension
        litellm_logging_obj.call_type = CallTypes.video_extension.value

        # Call the handler with _is_async flag instead of directly calling the async handler
        return base_llm_http_handler.video_extension_handler(
            video_id=video_id,
            video_extension_provider_config=video_extension_provider_config,
            custom_llm_provider=custom_llm_provider,
            litellm_params=litellm_params,
            logging_obj=litellm_logging_obj,
            extra_headers=extra_headers,
